...
export async function upsertProduct(p) {
  const { error } = await supabase.from('products').upsert({
    ...
    min_stock: p.minStock ?? 10, /* chốt 31/07: mặc định nhắc nhập là 10 */ sku: p.sku ?? '',
...
export async function updateOrderDB(id, patch) {
  ...
  /* v22 BigSeller: dấu đã đẩy + mã đơn bên BigSeller */
  if (patch.bigsellerExportedAt !== undefined) dbPatch.bigseller_exported_at = patch.bigsellerExportedAt
  if (patch.bigsellerOrderId    !== undefined) dbPatch.bigseller_order_id    = patch.bigsellerOrderId
...
/* v22: ghi CÓ MERGE. Đọc lại bản mới nhất của key từ DB rồi chồng
   `patch` lên, tránh việc hai tab (Trang chủ / Thương hiệu) cùng ghi
   key 'home' bằng snapshot cũ và xoá thay đổi của nhau — đây là lý do
   font "lưu xong lại mất". Chỉ dùng cho value là object (key 'home'). */
export async function mergeConfig(key, patch) {
  const { data } = await supabase.from('site_config')
    .select('value').eq('key', key).maybeSingle()
  const current = (data && data.value && typeof data.value === 'object') ? data.value : {}
...
/* ── v20: HOÀN KHO KHI HUỶ ĐƠN ────────────────────────────────────────────
   Trước đây bấm "Huỷ đơn" chỉ đổi chữ trạng thái, KHÔNG cộng kho lại →
   chạy vài tháng số kho sai bét. Hàm này cộng trả từng dòng hàng:
   - dòng là PHÂN LOẠI  → cộng kho phân loại đó
   - dòng là COMBO có BOM → cộng kho TỪNG MÓN CON theo BOM
       ('' = SP gốc, '*scent*' = mùi khách chọn lúc mua, còn lại = id phân loại)
   - combo KHÔNG có BOM (kiểu cũ) → cộng kho của chính combo
   - không khớp gì → cộng kho SP gốc
   GỌI 1 LẦN duy nhất khi chuyển sang Cancelled (caller phải kiểm
   trạng thái cũ !== Cancelled trước khi gọi, tránh cộng đôi). */
export async function restockOrder(order) {
  const items = Array.isArray(order?.items) ? order.items : []
  const byProd = {}
  for (const it of items) {
    if (!it?.productId) continue
    ;(byProd[it.productId] = byProd[it.productId] || []).push(it)
  }
  for (const pid of Object.keys(byProd)) {
    const { data: p, error } = await supabase
      .from('products').select('id, stock, variants, combos').eq('id', pid).single()
    if (error || !p) continue
    let stock    = Number(p.stock) || 0
    let variants = Array.isArray(p.variants) ? [...p.variants] : []
    let combos   = Array.isArray(p.combos)   ? [...p.combos]   : []
    const addVar = (vid, q) => {
      variants = variants.map(v => v.id === vid
        ? { ...v, stock: (Number(v.stock) || 0) + q } : v)
    }
    for (const it of byProd[pid]) {
      const q = Number(it.qty) || 0
      if (q <= 0) continue
      if (it.variantId && variants.some(v => v.id === it.variantId)) {
        addVar(it.variantId, q)
        continue
      }
      const c = it.variantId && combos.find(x => x.id === it.variantId)
      if (c) {
        const bom = Array.isArray(c.bom) ? c.bom : []
        if (bom.length) {
          for (const row of bom) {
            const rq = (Number(row.qty) || 1) * q
            if (row.variantId === '*scent*') { if (it.scentId) addVar(it.scentId, rq) }
            else if (row.variantId)          { addVar(row.variantId, rq) }
            else                             { stock += rq }
          }
        } else {
          combos = combos.map(x => x.id === c.id
            ? { ...x, stock: (Number(x.stock) || 0) + q } : x)
        }
        continue
      }
      stock += q
    ...
    await supabase.from('products')
      .update({ stock, variants, combos, updated_at: new Date().toISOString() })
      .eq('id', pid)
  }
}
function dbToOrder(r) {
  return {
    ...
    /* v22 BigSeller — chiều NẠP. Thiếu chỗ này thì ghi được mà đọc lại mất. */
    bigsellerExportedAt: r.bigseller_exported_at || null,
    bigsellerOrderId:    r.bigseller_order_id    || '',
