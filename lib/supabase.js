...
export async function upsertProduct(p) {
  const { error } = await supabase.from('products').upsert({
    ...
    tags: p.tags ?? '', story: p.story ?? '', img: p.img ?? '', img_mobile: p.imgMobile ?? '', tiktok_url: p.tiktokUrl ?? '', sort_order: p.sortOrder ?? Date.now(),
...
function dbToProduct(r) {
  return {
    ...
    flashEnd: r.flash_end, tags: r.tags, story: r.story, img: r.img, imgMobile: r.img_mobile ?? '', tiktokUrl: r.tiktok_url ?? '', sortOrder: r.sort_order ?? 0,
