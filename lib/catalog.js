...
function normalize(r) {
  ...
  const combos = (Array.isArray(r.combos) ? r.combos : []).map(c => ({
    ...
    img: c.img || '',
    /* v20 kho 1 nguồn: bom = combo GỒM những món nào trong kho.
       variantId '' = SP gốc, '*scent*' = mùi khách chọn, còn lại = id phân loại.
       Có bom thì "còn bán được" TÍNH từ kho món con, không dùng c.stock. */
    bom: (Array.isArray(c.bom) ? c.bom : []).map(b => ({
...
/* ── Phase 2: stock_meta (site_config) ───────────────────────────────────────
   { [productId]: { archived: bool, safety: { ''|variantId: số }, minStock: {...} } }
   - archived  → sản phẩm "nghỉ hưu": storefront KHÔNG hiện, KHÔNG đặt được,
                 nhưng không xoá sổ (đơn cũ vẫn đọc tên đúng từ items snapshot).
   - safety    → ngưỡng tự ngừng bán: web coi như hết hàng khi tồn ≤ ngưỡng.
   Cách áp RẺ NHẤT cho storefront: trừ ngưỡng NGAY TẠI ĐÂY — mọi chỗ ngoài
   trang bán (page.js / san-pham / ProductModal / giỏ) đang so `stock <= 0`
   và Math.min(stock) TỰ ĐÚNG, không phải sửa dòng nào. `stock` phía khách
   = "số còn ĐƯỢC BÁN", không phải số đếm trong kho. Admin đọc số thô qua
   lib/supabase (không đi qua đây) nên vẫn thấy số thật. */
async function fetchStockMeta() {
  try {
    const { data } = await sb.from('site_config')
      .select('value').eq('key', 'stock_meta').maybeSingle();
    return (data && typeof data.value === 'object' && data.value) || {};
  } catch { return {}; }
}
const cutSafety = (n, safety) => Math.max(0, (Number(n) || 0) - (Number(safety) || 0));
...
export async function fetchProducts() {
  ...
  return (data || [])
    .filter(r => !meta[r.id]?.archived)
    .map(r => {
      const p = normalize(r);
      const safe = meta[r.id]?.safety || {};
      p.stock = cutSafety(p.stock, safe['']);
      p.variants = p.variants.map(v => ({ ...v, stock: cutSafety(v.stock, safe[v.id]) }));
      return p;
    });
}
