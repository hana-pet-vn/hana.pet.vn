...
export const BS_STATUS_MAP = {
  ...
  /* Vá 31/07/2026 — file thật "Order-SKU-all" có 7 trạng thái, map cũ phủ 5:
     · 'Trả hàng & Hoàn tiền' → 'Return Check' ("Hoàn hàng — chờ kiểm"):
       KHÔNG tự hoàn kho (hàng chưa chắc về tay) — nhân viên kiểm xong xử tay.
     · 'Chờ xử lý' → không map ở đây; nằm trong BS_KEEP_STATUSES bên dưới. */
  'Trả hàng & Hoàn tiền': 'Return Check',
}

/* Trạng thái BS gặp trong file nhưng KHÔNG đổi gì trên web (đơn chưa in,
   không có gì để đẩy) — khác "trạng thái lạ" (unknown, cần bổ sung map). */
export const BS_KEEP_STATUSES = new Set(['Chờ xử lý'])
...
export function orderToBsRows(order, productMap, skuMap, opts = {}) {
  ...
  const out = rows.map(r => {
    ...
    /* Vá 18/08 (bằng chứng file mẫu + lỗi import thật): BS bắt tên tỉnh
       TRẦN không tiền tố — ví dụ của BS ghi "Bắc Kạn", còn "Thành phố Hà Nội"
       bị chê "Định dạng địa chỉ Tỉnh không chính xác". Quận/Phường GIỮ tiền tố
       (ví dụ BS: "Huyện Chợ Đồn", "Xã Bằng Phúc"). */
    o['Địa chỉ người nhận - Tỉnh'] = String(c.provinceName || '').replace(/^(Thành phố|Tỉnh)\s+/i, '')
...
export function planReconcile({ csvText, orders }) {
  ...
  for (const b of bsOrders.values()) {
    ...
    /* Vá 31/07: 'Chờ xử lý' = đơn chưa in bên BS → giữ nguyên trạng thái web */
    if (BS_KEEP_STATUSES.has(b.status)) { unchanged.push(o); continue }
    ...
    if (to !== 'Cancelled') {
      ...
      /* Vá 31/07: đơn đang "Hoàn hàng — chờ kiểm" là việc dính tiền-hàng,
         máy không tự kéo đi đâu nữa — người kiểm xong tự xử. */
      if (from === 'Return Check') { unchanged.push(o); continue }
    }
