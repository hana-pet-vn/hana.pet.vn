...
export function CartProvider({ children }) {
  ...
  const add = useCallback((item, qty = 1) => {
    ...
    const sid = item.scentId || '';
    setLines(prev => {
      /* v20: combo chọn mùi — cùng combo nhưng KHÁC mùi = 2 dòng riêng
         (gộp chung thì mất thông tin mùi để trừ kho + đóng hàng) */
      const i = prev.findIndex(l => l.productId === item.productId
        && (l.variantId || '') === vid && (l.scentId || '') === sid);
      if (i >= 0) {
        const next = [...prev];
