'use client'
// app/admin2/orders/page.js — Quản lý đơn hàng (Phase 1)
// ─────────────────────────────────────────────────────────────────────
...
import { doCancelOrder, doRevertToPending, doCreateGHN } from './_lib/actions'
...
function OrdersPage() {
  ...
  const setParams = useCallback((patch) => {
    ...
    const s = p.toString()
    router.replace('/admin2/orders' + (s ? `?${s}` : ''), { scroll: false })
  }, [sp, router])
  ...
  const createGHN = ctx?.role === 'owner'
    ? (o) => setModal({
        title: `Tạo vận đơn GHN cho ${o.code}?`,
        body: 'Book GHN THẬT ngay lập tức. Bình thường vận đơn tạo bên BigSeller — nút này chỉ để chữa cháy.',
        confirmText: 'Tạo vận đơn',
        onConfirm: () => doCreateGHN({ order: o, toast, setOrders }),
      })
    : null
