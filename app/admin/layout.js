...
// app/admin/layout.js
// ─────────────────────────────────────────────────────────────────────
// Khung chung /admin: topbar navy + sidebar kiểu Shopee Seller Center.
...
const NAV = [
  ...
  { href: '/admin/products',  label: 'Sản phẩm & Kho',     icon: '📦' },
  { href: '/admin/marketing', label: 'Kênh Marketing',     icon: '🎯', perm: 'manage_vouchers' },
  { href: '/admin/store',     label: 'Trang trí gian hàng', icon: '🎨', perm: 'edit_store' },
  { href: '/admin/settings',  label: 'Cài đặt',            icon: '⚙️', ownerOnly: true },
]
    ...
    ;(async () => {
      try {
        ...
        if (!['owner', 'staff'].includes(role)) {
          router.replace('/admin/login?err=norole'); return
        }
        ...
        <div style={{
          ...
          <aside style={{
            ...
            <div style={{
              ...
              <b style={{ color: T.navy }}>Admin chính thức</b><br />
              Kho / Marketing / Trang trí tạm dùng bản cũ tại /admin-cu tới khi Phase 2–4 chuyển xong.
            </div>
          </aside>
