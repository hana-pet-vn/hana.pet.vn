...
function OrdersPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!session) { router.replace('/admin-cu/login'); return }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f2f5fb', fontFamily:FONT_B }}>
      ...
      <div style={{ background:'#0d142e', padding:'0 24px', display:'flex', alignItems:'center', gap:14, height:56 }}>
        <button onClick={() => router.push('/admin-cu')} style={{ background:'transparent', border:'none', color:'rgba(255,255,255,0.6)', cursor:'pointer', fontFamily:FONT_T, fontSize:13 }}>← Trang chính</button>
